...
class my_minimum_variance_distortioless_response:
    ...
    def get_steering_vector(self, look_direction, anchor=1.5):
        '''
        ...
        # look_direction = look_direction * (-1)  # 这里为什么乘以-1
        for f, frequency in enumerate(frequency_vector):
            for m, mic_angle in enumerate(self.mic_angle_vector):
                steering_vector[f, m] = np.complex(np.exp((- 1j) * ((2 * np.pi * frequency) / self.sound_speed)
                                                          * ((anchor - m) * self.mic_diameter)
                                                          * np.cos(np.deg2rad(look_direction))))

        steering_vector = np.conjugate(steering_vector).T  # 计算共轭转置
        normalize_steering_vector = self.normalize(steering_vector)  # 归一化
    ...
    def get_steering_vector_16(self, look_direction, distance):
        '''
        :param look_direction:
        :param anchor: steering的锚点，默认值是阵列中心，即1.5，还可以取值0，1，2，3，分别对应于麦克风0 1 2 3
        :return:
        ...
        for f, frequency in enumerate(frequency_vector):
            for m, mic_angle in enumerate(self.mic_angle_vector):
                steering_vector[f, m] = np.complex(np.exp((- 1j) * ((2 * np.pi * frequency) / self.sound_speed) \
                                                          * (distance[m] * self.mic_diameter) \
                                                          * np.cos(
                    np.deg2rad(look_direction) - np.deg2rad(mic_angle))))  # 修改
    ...
    def get_mvdr_beamformer(self, steering_vector, R):
        ...
        for f in range(0, len(frequency_grid)):
            ...
            b = np.reshape(b, [1, 1])
            beamformer[:, f] = np.matmul(inv_R, steering_vector[:,
                                                f]) / b  # number_of_mic *1   = number_of_mic *1 vector/scalar
        return beamformer
